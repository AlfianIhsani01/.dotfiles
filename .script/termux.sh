...
validate_file() {
   ...
   if [[ ! -f "$file" ]]; then
      _log -t "File does not exist: $file"
      return 1
   fi

   # Check if file is readable
   if [[ ! -r "$file" ]]; then
      _log -e "File is not readable: $file"
      return 2
   fi

   # Check if file is writable
   if [[ ! -w "$file" ]]; then
      _log -e "File is not writable: $file"
      return 3
   fi
...
fix_linux_shebang() {
   ...
   [[ ${#files[@]} -eq 0 ]] && {
      _log -e "No files specified for Linux shebang fixing"
      return 1
   }

   _log -i "Fixing shebangs for Linux environment..."
   ...
   for file in "${files[@]}"; do
      ...
      if command -v realpath &>/dev/null; then
         ...
         target_file="$file"
         _log -t "realpath not available, using original path: $file"
      fi

      _log -t "Processing file: $target_file"
      ...
      temp_file=$(mktemp) || {
         _log -e "Failed to create temporary file for $target_file"
         ((files_failed++))
         continue
      }
      ...
      if ! cp "$target_file" "$backup_file"; then
         _log -e "Failed to create backup for $target_file"
         rm -f "$temp_file"
         ((files_failed++))
         continue
      fi
      ...
      if sed -E "1s|^#!${TERMUX_PREFIX}/bin/(.*)$|#!/bin/\1|" "$target_file" >"$temp_file"; then
         ...
         if ! cmp -s "$target_file" "$temp_file"; then
            if mv "$temp_file" "$target_file"; then
               _log -s "Fixed shebang in: $target_file"
               ((files_fixed++))
               changes_made=true
            else
               _log -e "Failed to apply changes to $target_file"
               mv "$backup_file" "$target_file" # Restore backup
               ((files_failed++))
            fi
         else
            _log -t "No shebang changes needed for: $target_file"
            ((files_skipped++))
         fi
      else
         _log -e "Failed to process shebang in $target_file"
         ((files_failed++))
      fi
...
fix_termux_shebang() {
   ...
   [[ ${#files[@]} -eq 0 ]] && {
      _log -e "No files specified for Termux shebang fixing"
      return 1
   }

   _log -i "Using termux-fix-shebang for Termux environment..."

   if ! command -v termux-fix-shebang &>/dev/null; then
      _log -e "termux-fix-shebang command not found"
      return 1
   fi
   ...
   for file in "${files[@]}"; do
      ...
      _log -t "Processing file with termux-fix-shebang: $file"
      ...
      if termux-fix-shebang "$file" 2>/dev/null; then
         _log -s "Fixed shebang in: $file"
         ((files_fixed++))
      else
         _log -e "Failed to fix shebang in: $file"
         ((files_failed++))
      fi
   done
...
fix_runtime_paths() {
   ...
   _log -i "Fixing runtime directory paths..."
   ...
   for file in "${files[@]}"; do
      if [[ -f "$file" ]] && [[ -w "$file" ]]; then
         _log -t "Fixing runtime paths in: $file"
         ...
         if sed -i "s|${search_pattern}|${replacement_pattern}|g" "$file"; then
            _log -s "Fixed runtime paths in: $file"
            rm -f "$backup_file"
         else
            _log -e "Failed to fix runtime paths in: $file"
...
fix_shebangs() {
   ...
   if [[ ${#files[@]} -eq 0 ]]; then
      _log -i "No files specified, using default shell configuration files"
      ...
      if [[ ${#files[@]} -eq 0 ]]; then
         _log -w "No shell configuration files found"
         return 0
      fi

      _log -i "Found ${#files[@]} shell configuration files: ${files[*]}"
   fi
   ...
   linux | android)
      ...
      ;;
   *)
      _log -e "Unknown environment type: $env_type"
      return 1
      ;;
   esac
   ...
   _log -i "Files processed: $files_processed"
   _log -s "Files fixed: $files_fixed"
   [[ $files_skipped -gt 0 ]] && _log -w "Files skipped: $files_skipped"
   [[ $files_failed -gt 0 ]] && _log -e "Files failed: $files_failed"
...
main() {
   ...
   while [[ $# -gt 0 ]]; do
      ...
      --dry-run)
         dry_run=true
         _log -i "Dry run mode - no changes will be made"
         shift
         ;;
      -*)
         _log -e "Unknown option: $1"
   ...
   if [[ -n "$env_override" ]]; then
      _log -i "Environment overridden to: $env_override"
   ...
   if [[ -n "$shell_override" ]] && [[ ${#files[@]} -eq 0 ]]; then
      ...
      _log -i "Using $shell_override shell files: ${files[*]}"
   fi

   # Dry run mode
   if [[ "$dry_run" == true ]]; then
      _log -i "Dry run - showing files that would be processed:"
...
fix_linux_shebang_legacy() {
   _log -w "Using legacy function name. Please use 'fix_shebangs' instead."
   fix_linux_shebang "$@"
}
